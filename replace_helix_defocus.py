...
micrograph_particles={name.split("/")[-1].split(".")[0]:list(micrograph_name) for name,micrograph_name in groupby(particles,lambda x:x[MicrographNameindex])}
...
for local_star in local_stars:
    ...
    try:
        ...
        helical_particles={ID:list(helix) for ID, helix in groupby(micrograph_particle,lambda x:x[helicalIDindex])}
        ...
        #Record helix ID number
            ...
            #Record the linear difference according to the defocus A,U,Vstart and end of the helix
                ...
                #Update the defocus U,V,A for the helical segments
                    helical_particles[ID][ii][DefocusUindex]=str(float(local_ps[local_ID][local_DefocusUindex])+diffU*ii)
                    helical_particles[ID][ii][DefocusVindex]=str(float(local_ps[local_ID][local_DefocusVindex])+diffV*ii)
                    helical_particles[ID][ii][DefocusAindex]=str(float(local_ps[local_ID][local_DefocusAindex])+diffA*ii)
                new_micrograph_particles.extend(helical_particles[ID])
